...
class Game {
    constructor() {
        ...
        this.draggedFromPlayer = null;
        this.gameOver = false;
        this.minionsPlayedThisPhase = false; // Track for Ninja Acolyte
        this.effectsManager = new EffectsManager();
        this.ui = new UIManager(this);
    }
    setupGame(player1Factions, player2Factions) {
        this.players = [
        ...
        this.bases = shuffledBases.slice(0, 3).map((baseData, index) => (Object.assign(Object.assign({}, baseData), { id: `base-${index}`, originalBreakpoint: baseData.breakpoint, minions: [], attachedActions: [] })));
    ...
    createPlayer(id, name, factions) {
        const deck = [];
        factions.forEach(faction => {
            const factionCards = cardDatabase[faction];
            factionCards.forEach(card => {
                const cardCopy = Object.assign(Object.assign({}, card), { id: `${card.id}-p${id}` });
                if (card.type === 'minion') {
                    cardCopy.basePower = card.power;
                    cardCopy.attachedActions = [];
                }
                deck.push(cardCopy);
            });
        });
    ...
    playCard(card, targetId, playerId) {
        ...
        if (card.type === 'minion') {
            this.playMinion(card, targetId, playerId);
            this.minionsPlayedThisTurn++;
            this.minionsPlayedThisPhase = true;
        }
        else {
            this.playAction(card, targetId, playerId);
            this.actionsPlayedThisTurn++;
        }
        this.checkBases();
        this.ui.render();
    }
    playMinion(minion, baseId, playerId) {
    ...
    triggerMinionEffect(minion, base, playerId) {
        const player = this.players.find(p => p.id === playerId);
        if (!player)
            return;
        switch (minion.name) {
    ...
    playAction(action, targetId, playerId) {
        const player = this.players.find(p => p.id === playerId);
        if (!player)
            return;
        if (action.ongoing) {
    ...
    attachOngoingAction(action, targetId, playerId) {
    ...
    executeActionEffect(action, targetId, playerId) {
        const player = this.players.find(p => p.id === playerId);
        if (!player)
            return;
        switch (action.name) {
            ...
            case 'Rampage':
                this.ui.promptSelectBase(playerId, 'Choose a base to reduce breakpoint', (baseId) => {
                    ...
                    this.ui.promptSelectFromList('Choose your minion', yourMinions.map(m => ({
    ...
    moveMinion(minionId, fromBaseId, toBaseId) {
    ...
    getMinionPower(minion, base) {
        var _a;
        let power = minion.basePower || minion.power || 0;
        // Attached actions
        (_a = minion.attachedActions) === null || _a === void 0 ? void 0 : _a.forEach(action => {
            if (action.name === 'Upgrade') {
                power += 2;
            }
            else if (action.name === 'Poison') {
    ...
    scoreBase(base) {
        ...
        const playerPowers = this.players.map(player => ({
            playerId: player.id,
        ...
        const unusedBases = basesData.filter(bd => !this.bases.some(b => b.name === bd.name));
        if (unusedBases.length > 0) {
            const newBaseData = unusedBases[Math.floor(Math.random() * unusedBases.length)];
            this.bases[baseIndex] = Object.assign(Object.assign({}, newBaseData), { id: `base-${Date.now()}`, originalBreakpoint: newBaseData.breakpoint, minions: specialMinions, attachedActions: [] });
        }
        else {
